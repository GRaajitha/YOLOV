...
class OVIS(Dataset):
    ...
    def load_anno_from_ids(self, id_):
        ...
        #im_ann['name'] = im_ann['name']#self.coco.dataset['seq_dirs'][im_ann['sid']]+ '/' + im_ann['name']
...
#OVIS(data_dir='/opt/dataset/OVIS/',json_file='ovis_train.json')
...
def convert_ovis_coco(data_dir = '/opt/dataset/OVIS/annotations_train.json',save_dir = '/opt/dataset/OVIS/ovis_train.json'):
    ...
    id = 0
    img_id_dic = {}
    for sid,vid in enumerate(ovis['videos']):
        w,h = vid['width'],vid['height']
        for fid,frame_name in enumerate(vid['file_names']):
            img_id_dic[str(sid) + '_' + str(fid)] = id
            tmp_dic = {
                'id':id,
                'sid':sid,
                'fid':fid,
            ...
            ovis_vid['images'].append(tmp_dic)
            id += 1
    id = 0
        ...
        for i in range(len(anno['bboxes'])):
            if anno['bboxes'][i] != None:
                tmp_dic = {
                    ...
                    'bbox':anno['bboxes'][i],
                    'area':anno['areas'][i],
                    'image_id': img_id_dic[str(anno['video_id']-1) + "_" + str(i)]
                }
                id += 1
